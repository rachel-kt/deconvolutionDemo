# -*- coding: utf-8 -*-
"""
Created on Fri Feb 26 15:22:56 2021

pygad tweaked by Maria to include tolerance function
...
class GA:
    ...
    def run(self):
        ...
        for generation in range(self.num_generations):
            ...
            self.change=10000000
            # trying to add the tolfun
            if self.generations_completed > self.Window:
                self.Bestfvals = self.best_solutions_fitness[self.generations_completed-self.Window:]
                self.change=numpy.abs(self.Bestfvals[0]-self.Bestfvals[-1])/(self.Window*max(1,numpy.abs(self.Bestfvals[-1])))

            if self.change <= self.TolFun :
                print('ga stopped because the TolFun was reached at generation = {generation}'.format(generation=self.generations_completed))
                print('change = {change}'.format(change=self.change))
